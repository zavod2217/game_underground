...
def random_position(map, objects):
    position = [random.randint(1, 39), random.randint(1, 39)]
    loop = True
    while loop:
        loop = False
        if map[position[1]][position[0]] == wall:
            loop = True
            position = [random.randint(1, 39), random.randint(1, 39)]
        for obj in objects:
            if position == obj.position or position == (1, 1):
                loop = True
                position = [random.randint(1, 39), random.randint(1, 39)]
...
class MapFactory(yaml.YAMLObject):
    ...
    #FIXME create_map, create_object
    @classmethod
    def create_map(cls):
        pass
            ...
            for i in range(41):
                for j in range(41):
                    if i == 0 or j == 0 or i == 40 or j == 40:
                        ...
                        if i < 2 or j < 2:
                            self.Map[j][i] = random.choices([floor1, floor2, floor3])[0]
                        else:
                            self.Map[j][i] = random.choices([wall] + [floor1, floor2, floor3]*2)[0]
            ...
            for obj_name in object_list_prob['objects']:
                ...
                for i in range(random.randint(prop['min-count'], prop['max-count'])):
                    ...
                    self.objects.append(Objects.Ally(
                        prop['sprite'], prop['action'], position))
            ...
            for obj_name in object_list_prob['enemies']:
                ...
                for i in range(random.randint(0, 5)):
                    ...
                    self.objects.append(Objects.Enemy(
                        prop['sprite'], prop, prop['experience'], position))
...
def service_init(sprite_size, full=True, engine=None):
    ...
    if engine:
        engine.hero.transform = False
        engine.hero.direction = 'right'
        shield = False
